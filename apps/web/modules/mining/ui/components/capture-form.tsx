...
export function CaptureForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (capture, values) => {
      ...
      reset({ kind: values.kind, originalSentence: "", source: "", text: "" });
    ...
    <section aria-labelledby="capture-title">
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
        ...
        <div className="space-y-2">
          <Label htmlFor="capture-text">{kind === "sentence" ? t("sentenceToRemember") : t("target")}</Label>
          <Input
            id="capture-text"
            autoCapitalize="none"
            autoCorrect="off"
        ...
        {kind === "term" ? <div className="space-y-2">
